...
test.describe("Staff - Contract List @regression @e2e", () => {
  ...
  test.beforeEach(async ({ page, adminApi }) => {
    ...
    await new NavigationPage(page).open("/staff/contracts");
  });
  ...
  test("[E2E-STF-CTR-002] - Staff Contract List - Contract Filter - Customer Building and Status Filtering", async ({ page }) => {
    const contractPage = new StaffContractListPage(page);
    await contractPage.expectLoaded();
  ...
  test("[E2E-STF-CTR-003] - Staff Contract List - Contract Details - Details Modal from List", async ({ page }) => {
    const contractPage = new StaffContractListPage(page);
    await contractPage.expectLoaded();
    await contractPage.waitForTableData();
